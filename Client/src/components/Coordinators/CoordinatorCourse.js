...
const CoordinatorCourse = () => {
  ...
  return (
    <div className="p-6">
      ...
      <Link to="/coordinator/courses/new" className="bg-blue-500 text-white p-2 rounded">Add New Course</Link>
      {courses.length === 0 ? (
        ...
        <ul className="mt-4">
          {courses.map(course => (
            <li key={course._id} className="bg-white p-4 mb-4 rounded shadow">
              ...
              <Link to={`/coordinator/courses/update/${course.courseId}`} className="bg-yellow-500 text-white p-2 rounded mr-2">Edit</Link>
...
export default CoordinatorCourse;
