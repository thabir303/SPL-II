...
const CoordinatorEditClassSlot = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchClassSlot = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/class-slots/${id}`
        );
        setFormData(response.data);
      } catch (error) {
        setError("Failed to fetch class slot");
        toast.error("Failed to fetch class slot", { autoClose: 2000 });
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    const fetchAssignedCourses = async () => {
      if (formData.teacherId) {
        try {
          const response = await axios.get(
            `http://localhost:5000/api/courses/teacher/${formData.teacherId}`
          );
        ...
        setAssignedCourses([]); // Reset assignedCourses to an empty array if teacherId is not set
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("Class slot updated successfully", { autoClose: 2000 });
      ...
      if (error.response && error.response.data) {
        ...
        toast.error("Failed to update class slot", { autoClose: 2000 });
      }
    }
  };
...
export default CoordinatorEditClassSlot;
