        ...
        <form onSubmit={handleSubmit}>
          {formData.courses.map((course, index) => (
            <div key={index} className="mb-4">
              ...
              />
              <span>Active</span>
              <button type="button" onClick={() => removeCourseField(index)} className="bg-red-500 text-white p-2 rounded mt-2">Remove</button>
            </div>
          ))}
          <button type="button" onClick={addCourseField} className="bg-blue-500 text-white p-2 rounded mb-4">Add Another Course</button>
          <button type="submit" className="w-full p-2 bg-green-500 text-white rounded">Assign Courses</button>
        </form>
      </div>
    </div>
  );
...
export default CoAssignCourses;
