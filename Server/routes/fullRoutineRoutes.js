...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (existingFullRoutine) {
      return res.status(400).json({ error: "Routine for the same section and batch number within the same semester already exists" });
    }
...
//       return res.status(404).json({ error: "Full  not found" });
//     }
