...
router.post('/', async (req, res) => {
  try {
    const { batchNo, semesterName } = req.body;

    // Validate input data
    if (!batchNo || !semesterName) {
    ...
    const newBatch = new Batch({ batchNo, semesterName });
    const savedBatch = await newBatch.save();
    res.json({ message: 'Batch assigned successfully', data: savedBatch });
  } catch (error) {
    console.error('Error creating batch:', error);
    res.status(500).json({ error: 'Batch already running on a semester. You can Edit the Batch No or delete the Batch No' });
  }
});
...
router.put('/:batchNo', async (req, res) => {
  try {
    ...
    // Check if the same semesterName is assigned to different batches
    const existingBatch = await Batch.findOne({ semesterName, batchNo: { $ne: batchNo } });
    if (existingBatch) {
      return res.status(400).json({ error: 'The same semester name cannot be assigned to different batches' });
    }

    // Update the batch
    const updatedBatch = await Batch.findOneAndUpdate(
      { batchNo },
      { semesterName, isActive },
      { new: true }
    );
